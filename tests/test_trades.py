...
from trades import load_trade_cache, parse_trade_dataframe, save_trade_cache, update_trade_cache
...
class TradeStatementTests(unittest.TestCase):
    def test_uploaded_statement_is_parsed_without_a_fixed_local_path(self):
        ...
        rows = [
            ["电子对账单"] + [None] * 12,
            header,
            ["20260701", "人民币", "private", "563360", "A500", "证券买入", 100, 1.0, 0, 0, 0, -100, 0],
            ["20260702", "人民币", "private", "563360", "A500", "证券卖出", 100, 1.1, 0, 0, 0, 110, 0],
        ]
    ...
    def test_new_statement_replaces_previous_cache(self):
        ...
        new_trades = {"510300": [{"date": pd.Timestamp("2026-07-08"), "type": "buy", "price": 4.0, "qty": 50, "amount": 200.0}]}
        with tempfile.TemporaryDirectory() as directory:
            path = os.path.join(directory, "trades.json")
            save_trade_cache(old_trades, path)
            with patch("trades.parse_statement", return_value=new_trades):
                update_trade_cache(object(), path)
            loaded = load_trade_cache(path)
