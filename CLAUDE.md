...
- 当前用户是达达。
- **交易日志自动记录**：每次跟达达聊完 ETF 盘面（分析、预测、操作计划），自动调用 `journal.create_entry()` 写入日志。不用等达达说"记录一下"。日志路径：`journal/{symbol}/{date}.md`。
  - 达达的预测和分析可能比较啰嗦，凝练后记录，但绝不美化、不修正、不为了让预测贴近实际而改动原文意思。真实第一，达达要校准自己的判断力。
- 项目模式：`standard`。
